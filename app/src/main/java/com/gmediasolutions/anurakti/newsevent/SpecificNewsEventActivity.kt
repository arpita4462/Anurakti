...
//      setup toolbar
    ...
    private fun getspecificnews(getnewsId: String?) {
        ...
        val call = apiService.getNews(getnewsId!!)
        call.enqueue(object : Callback<SpecificNewsModel> {
            ...
            override fun onResponse(call: Call<SpecificNewsModel>, response: Response<SpecificNewsModel>) {
                if (response.code() == 401) {
                    ...
                    ne_catone.text = mnews!!.data.get(0).category
                    ne_headone.text = mnews.data.get(0).heading
                    if (mnews.data.get(0).photos.equals("")) {
                        ne_imgone.visibility = View.GONE
                    } else {
                        ne_imgone.visibility = View.VISIBLE
                        Glide.with(this@SpecificNewsEventActivity).load(mnews.data.get(0).photos)
                            .centerCrop()
                            .into(ne_imgone)
                    }
                    ne_context1.text = mnews.data.get(0).content1
                    ne_context2.text = mnews.data.get(0).content2
