...
import kotlinx.android.synthetic.main.fragment_friends_frnds.*
...
class FriendFriendsFragment : Fragment(), NetworkStateReceiver.NetworkStateReceiverListener,
    ...
    companion object {
        fun newInstance(): FriendFriendsFragment = FriendFriendsFragment()
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val myView = inflater.inflate(R.layout.fragment_friends_frnds, container, false)

        //        initialize the layout view
        frindlistRecycleView = myView.findViewById(R.id.frndlist_recycleViewfrnds)
        searchfrindlist = myView.findViewById(R.id.serach_frndlistfrnds)
            ...
            override fun onResponse(call: Call<GetFriendModel>, response: Response<GetFriendModel>) {
                if (response.code() == 401) {
                    ...
                    if (frndlists != null) {
                        ...
                        if (frindlist!!.size.equals(0)) {
                            tv_urfrndlist.text = getString(R.string.no_frnd)
                        } else {
                            tv_urfrndlist.text = getString(R.string.frnd_list)
                        }
                        frindlistFilterSerch.notifyDataSetChanged()
                    } else {
                        spotdialog!!.dismiss()
